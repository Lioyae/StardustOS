...
1. **本函数在关中断状态下被调用**（"关中断 → 查队列确实为空且无到期项 → 执行空闲 → 恢复中断"）。所以 `star_idle()` 要极短（几条寄存器操作），**不要在里面对 EA 做任何操作**——挂着牌子闭眼，门铃响了自然会醒。
2. **tick 必须持续运行**。唤醒依赖 Timer0 中断，最坏唤醒延迟 = 1 个 tick。若你的应用没使能 ET0（或想绝对安全），可在编译选项定义 `STAR_PORT_IDLE_NOOP`，让 `star_idle()` 变成空转不休眠（省电没了，但绝不会睡过头）。
3. **8051/251 目前只有固定拍，没有 tickless**（`STAR_TICKLESS=1` 是其它架构的参考实现）。固定拍已足够：没事时芯片每 1ms 醒一次看一眼就接着睡，电流已经很低。tickless 省的那点电，等内核后续版本支持再上，本文不展开实现细节。
...
**Q6：空闲没省电？**
用电流表测：空闲时电流应明显下降。没降？检查 `star_idle()` 是否真的执行了（`star_port.c` 有没有被你自己写的同名函数顶掉）、有没有误定义 `STAR_PORT_IDLE_NOOP` 把空闲关成空转。另外确认 `ET0=1`：IDL 空闲靠中断唤醒，中断没使能时内核宁可空转（见 4.2 契约 2）。
