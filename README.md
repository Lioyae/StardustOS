...
StardustOS 是面向小容量单片机的 C 语言事件驱动调度框架——一个协作式内核（事件队列 + 软定时器 + 邮箱 + 可选任务层，非抢占式 RTOS），聚焦 STC 系列 8051 / 80251 单片机（默认配置内核本体仅约 280B RAM）。
...
| 低功耗 | deadline 感知：`star_next_due()` 暴露最近到期时刻，空闲时队列空且无到期项才进 `star_idle(next_due)`；8051/251 默认空转（安全），定义 `STAR_PORT_IDLE` 启用 PCON IDL 空闲（star_idle 自动开/关中断处理唤醒，事件最多延迟 1 拍）。tickless（`STAR_TICKLESS=1`）8051/251 无实现，暂不支持 |
