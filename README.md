...
- 支持 8051（STC8H/8A、STC89C52、STC8051U/8052U，Keil C51 / SDCC）、80251（STC32G，Keil C251）与宿主机（x86）；中断延迟 = tick 中断 + 内核临界区（事件入队 O(1)；邮箱拷贝与 item_size 成正比；post_replace 与队列长度成正比）。临界区时长随配置与主频变化，**需按平台实测**（估算公式与实测方法见 [使用教程附录 A](docs/usage.md)）
